...
static NOINLINE void draw_texture_rectangle(
    ...
    s32 xl, s32 yl, s32 xh, s32 yh,
    i32 s, i32 t, s16 dsdx, s16 dtdy
);
...
void process_RDP_list(void)
{
    int length;
    unsigned int offset;
    const u32 DP_CURRENT = GET_RCP_REG(DPC_CURRENT_REG) & 0x00FFFFF8;
    const u32 DP_END     = GET_RCP_REG(DPC_END_REG)     & 0x00FFFFF8;
    ...
    *GET_GFX_INFO(DPC_STATUS_REG) &= ~DP_STATUS_FREEZE;
    ...
    if (GET_RCP_REG(DPC_STATUS_REG) & DP_STATUS_XBUS_DMA)
        do
        {
...
static void set_convert(void)
{
    ...
    command_word =
        ((u64)cmd_data[cmd_cur + 0].UW32[0] << 32)
      | ((u64)cmd_data[cmd_cur + 0].UW32[1] <<  0)
    ;
    k_YUV_RGB[0] = (u32)(command_word >> 45) & 0x1FF;
    k_YUV_RGB[1] = (u32)(command_word >> 36) & 0x1FF;
    k_YUV_RGB[2] = (u32)(command_word >> 27) & 0x1FF;
    k_YUV_RGB[3] = (u32)(command_word >> 18) & 0x1FF;
    k_YUV_RGB[4] = (u32)(command_word >>  9) & 0x1FF;
    k_YUV_RGB[5] = (u32)(command_word >>  0) & 0x1FF;

/*
 * Force the preparation of k0, k1, k2, and k3 as 9-bit sign-extended
 * integers, since these are only ever accessed by the RDP as such.
 */
    for (i = 0; i < 4; i++)
        k_YUV_RGB[i] = SIGN(k_YUV_RGB[i], 9);

/*
 * Extra credit for performance:  During the texture filter unit, these
 * values are applied in the following equation:
 *     (k_YUV_RGB[i] - ~k_YUV_RGB[i])
 *
 * On a 2's complement CPU, that can be simplified to:
 *     (k_YUV_RGB[i] << 1) + 1
 */
    for (i = 0; i < 4; i++)
        k_YUV_RGB[i] = 2 * k_YUV_RGB[i] + 1;
    return;
}
...
static NOINLINE void draw_triangle(int shade, int texture, int zbuffer)
{
    ...
    i32 d_rgba_dx[4]; /* RGBA delta per x-coordinate delta */
 ...
 */
    ...
    sign_dxhdy = (cmd_data[base + 2].W32[1] < 0);
    ...
    for (k = ycur; k <= ylfar; k++)
    {
        ...
        else
        {
            ...
            xlrsc[1] =
                curunder ? clipxhshift : ((xlr[1] >> 13) & 0x3FFE) | stickybit;
            ...
            xlrsc[0] =
                curunder ? clipxhshift : ((xlr[0] >> 13) & 0x3FFE) | stickybit;
